...
# This script is set up to fit a crawl model to several 
# deployments at once.
...
tbl_data <- tbl_data %>% 
  ...
  dplyr::mutate(pred_pts = purrr::map(fit, 
                                      ~crwPredict(.x, predTime = "4 hours", return.type = 'minimal')))
...
final <- pred_data %>%
  select(animal, date, latitude, longitud, se.mu.x, se.mu.y, -geometry)


## write csv
write.csv(final, "ZcTag002-044_crawl_4hTimeStep_2020Aug04.csv", row.names = F)
