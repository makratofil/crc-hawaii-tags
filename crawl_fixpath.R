...
##              crawl fix path:
##      adjust crawl fitted trajectory
##                around land
...
library(tidylog)
library(lubridate)
library(crawl) # need to install devel-old-fix-path branch from github
library(ptolemy)
library(mapview)
library(purrr)
library(ggplot2)
...
tbl_locs <- readr::read_csv("Douglas Filtered/ZcTag002-044_DouglasFiltered_KS_r10d3lc2_2020MAYv2.csv",
...
## set error ellipse for deployment (known) locations 
tbl_locs <- tbl_locs %>%
  mutate(
...
tbl_data[[3]][[1]] # example, first tag 

## predict locations at 4 hour time interval.
## if want standard error and speed (m/h) estimates,
## specify return.type = 'flat'. Otherwise, don't
## specify or use 'minimal'.
tbl_data <- tbl_data %>% 
  dplyr::filter(map_lgl(fit, ~inherits(.x,"crwFit"))) %>% 
  dplyr::mutate(pred_pts = purrr::map(fit, 
                                      ~crwPredict(.x, predTime = "4 hours", return.type = 'flat')))
...
## Fit predicted path around land

## first get the number of points that intersect with land
tbl_data %>%
  select(animal, pts_sf) %>%
  unnest() %>%
  st_as_sf(crs = 3750) %>%
  st_intersects(map_base) %>%
  purrr::map_lgl(~ length(.x) > 0) %>%
  sum()

## fix internal crawl fix_path() function
## open the script "crawl_fixpath_mak.R" and run 
## each function (some custom modifications to the 
## original function were made)


## re-route portions of the predicted path around land
tbl_data <- tbl_data %>%
  mutate(fix = purrr::map2(
    pred_pts,
    fit,
    ~ fix_path(.x, 
              vector_mask = map_base,
              crwFit = .y)
  ))

## update nested tibble with fixed columns
tbl_data <- tbl_data %>%
  mutate(
    pts_fix = purrr::map(fix, ~ crw_as_sf(.x, ftype = "POINT",
                                          locType = c("p","f"))),
    line_fix = purrr::map(fix, ~ crw_as_sf(.x, ftype = "LINESTRING",
                                           locType = c("p","f")))
  )

## examine number of points that may still intersect with polygon
tbl_data %>%
  select(animal, pts_fix) %>%
  unnest() %>%
  st_as_sf(crs = 3750) %>%
  st_intersects(map_base) %>%
  purrr::map_lgl(~ length(.x) > 0) %>%
  sum() # apparently 2 more points that still intersect with land polygon

## map it 
tbl_data %>%
  select(animal, line_fix) %>%
  unnest() %>%
  select(-id) %>%
  st_as_sf(crs = 3750) %>%
  st_transform(4326) %>% st_to_360() %>%
  mapview(map.types = "Esri.OceanBasemap", zcol = "animal",
          burst = TRUE, legend = FALSE, homebutton = FALSE)

## the two remaining points on land are from ZcTag006, and they are very close to shore within the Hilo Bay 
## region. Likely a result of buffer size (1000 meters here) or gaps between predicted locations.

## If want to extract coordinates and save file:
...
final <- pred_data %>%
  select(animal, date, latitude, longitud, -geometry)


## write csv
write.csv(final, "SSM/ZcTag002-044_crawl_4hTimeStep_FixPath_2020Aug10.csv", row.names = F)
