...
surfs <- dplyr::filter(behavior, What == "Surface")
bh = surfs[0,]

for (i in 1:nrow(surfs)) {
  cat("\r", i, "                     ")
  bh = rbind(bh, surfs[i, ])
  dt0 = surfs[i, "datetimeHST"]
  dt1 = as.POSIXct(format(surfs[i, "End"], tz="Pacific/Honolulu"), tz="Pacific/Honolulu")
  sr = surfs[i, "sunrise"]
  ss = surfs[i, "sunset"]
  sunr = F
  if ((dt0 < sr) && (dt1 > sr)) {           # If start < sunrise, and end > sunrise, sunrise is in this segment
    ...
    bh[nrow(bh), "End"] = as.POSIXct(format(surfs[i, "sunrise"], tz="UTC"), tz="UTC") # make end time record = sunrise
    bh = rbind(bh, surfs[i, ]) # bind back to behavior
    bhrow = nrow(bh)
    bh[bhrow, "Start"] = as.POSIXct(format(surfs[i, "sunrise"], tz="UTC"), tz="UTC") # start of split record = sunrise 
    ...
    bh[bhrow, "aspect"] = NA
    bh[bhrow, "slope"] = NA
    bh[bhrow, "datetimeUTC"] = as.POSIXct(format(surfs[i, "sunrise"], tz="UTC"), tz="UTC")
    bh[bhrow, "datetimeHST"] = surfs[i, "sunrise"]
    bh[bhrow, "isDusk"] = F
  ...
  if ((dt0 < ss) && (dt1 > ss)) {    # if start < sunset, and end > sunset, sunset is in this segment
    ...
    bh[nrow(bh), "End"] = as.POSIXct(format(surfs[i, "sunset"], tz="UTC"), tz="UTC")
    bh = rbind(bh, surfs[i, ])
    bhrow = nrow(bh)
    bh[bhrow, "Start"] = as.POSIXct(format(surfs[i, "sunset"], tz="UTC"), tz="UTC")
    if (sunr) {
    ...
    bh[bhrow, "aspect"] = NA
    bh[bhrow, "slope"] = NA
    bh[bhrow, "datetimeUTC"] = as.POSIXct(format(surfs[i, "sunset"], tz="UTC"), tz="UTC")
    bh[bhrow, "datetimeHST"] = surfs[i, "sunset"]
    bh[bhrow, "isDusk"] = F
    bh[bhrow, "secNearestSunriseset"] = 0
    bh[bhrow, "sunAzimuth"] = NA
    bh[bhrow, "sunAltitude"] = NA
...
## bind back with dives
dives <- dplyr::filter(behavior, What == "Dive")
all <- dplyr::bind_rows(dives, bh) %>%
  dplyr::arrange(datetimeUTC)

## calculate record durations
all$duration = as.duration(all$Start %--% all$End)
all$durationSecs = as.integer(all$duration)
all$durationDays = all$duration / ddays(1)

## save file 
write.csv(all, paste0("GIS output/Dive pseudotracks/",Animal, "_BehPos_crawl_5minStep_FixedPath_",
                     format(Sys.time(), "%Y%m%d"), "_correct.csv"), row.names=F)
