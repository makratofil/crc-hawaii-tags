## crawl fix_path() functions, adjusted by MAK (Michaela A. Kratofil).
## Updated: 11 AUG 2020

## Changes made:
    # added list() around path_prediction_fix(), line 257
    # changed barrier_buffer = 500 meters instead of 50 (may have to be adjusted), line 59
...
#' @return a tibble with each record identifying the segments and pertinent values
#' @export

fix_segments <- function(crw_sf, vector_mask, barrier_buffer=500, crwFit, 
                         alpha, crwIS = FALSE, quiet = FALSE) {
  ...
  for (i in 1:nrow(segments)) {
    ...
    if (!crwIS) {
      segments[[i,"fixed_seg"]] <- list(path_prediction_fix(coast_points, 
                                                       sigma = exp(par[1]), 
                                                       beta = exp(par[2])))
    }
...
fix_path <- function(crw_object, vector_mask, crwFit, quiet = FALSE, random=TRUE) {
